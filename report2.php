...
$schools = array();
 while($row = $result->fetch_array()) {
    $sql = "SELECT *, count(students.student_id) AS students_count, ROUND(AVG(students.student_grade),2) as average_grade FROM students INNER JOIN classes ON students.class_id = classes.class_id  WHERE classes.school_id=". $row['school_id'] . " GROUP BY class_name ORDER BY class_name ASC"; 
      $schools = [];
